...
class weather(object):
	...
	def getWeatherInfo(self):
		code = getcode(self.city)
		api_url = 'http://t.weather.sojson.com/api/weather/city/{}'.format(code)
		response = requests.get(api_url).text
		selector = json.loads(response)
		self.weatherInfo = selector['data']['forecast'][0]
	...
	def template(self):
        ...
        def sendTemperature(self):
                self.message.append("今天是%s %s\n" % (self.weatherInfo["ymd"],self.weatherInfo["week"]))
                self.message.append("%s最高温度 %s，最低温度 %s\n" % (self.city, self.weatherInfo["high"].split(" ")[1],self.weatherInfo["low"].split(" ")[1]))
        def sendWeatherType(self):
